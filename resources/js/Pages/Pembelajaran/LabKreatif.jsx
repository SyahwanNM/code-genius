...
import {
    ...
    Sparkles, Wifi, X, Zap, AlertCircle, CheckCheck, HelpCircle
} from 'lucide-react';
...
export default function LabKreatif({ auth, templates }) {
    ...
    const mintaHintAI = async () => {
        if (!selectedTemplate) return;
        const step = selectedTemplate.steps[stepAktif];
        setAiOpen(true);
        setAiLoading(true);
        setAiData(null);
        try {
            const res = await axios.post('/lab/evaluasi', {
                ...
                nama_proyek:    selectedTemplate.nama,
                step_aktif:     step.judul,
                deskripsi_step: step.deskripsi,
            });
            setAiData(res.data);
        } catch (err) {
            ...
            <header className="h-14 border-b border-white/5 flex items-center gap-2 px-3 lg:px-5 bg-[#0B0E14] shrink-0">
                ...
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 text-sm min-w-0">
                        <span className="font-bold text-white truncate shrink-0 max-w-[120px] sm:max-w-none">
                ...
                {/* Progres badge */}
                <div className="shrink-0 hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-accent/10 text-accent border border-accent/20 text-[10px] font-black uppercase tracking-widest">
                    <Sparkles size={12} />
                        ...
                        {selectedTemplate.steps.map((step, idx) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full text-left p-4 rounded-2xl border transition-all group/step ${
                                        isActive
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        <div className={`w-6 h-6 rounded-full border flex items-center justify-center shrink-0 mt-0.5 text-[10px] font-black transition-all ${
                                            isDone
                                    ...
                                    {isActive && (
                                        <button
                                            ...
                                            className={`mt-3 w-full py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                                                isDone
